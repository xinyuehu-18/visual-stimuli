...
folders <- dir(path="~/Desktop/xhuThesis/style")
for (i in folders){
  
  stylenames <- i
  base_image_path <- "13.JPG"
  style_reference_image_path <-paste0("~/Desktop/xhuThesis/style/", stylenames)
  ...
  for(j in 1:iterations){
    ...
    # plot
      ...
      if(j == 15){
      jpeg(file=paste0("~/Desktop/xhuThesis/no13/13", stylenames))
      plot(as.raster(im))
      dev.off()
